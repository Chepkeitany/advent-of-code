def sum_next_number_in_sequence(sequences):
    ...
    for sequence in sequences:
        ...
        # by extrapolating backwards i.e filling in the first values for each previous sequence
        next_number = 0
        for i in range(len(temp_sequences) - 1, -1, -1):
            temp_sequence = temp_sequences[i]
            # print(temp_sequence[0] - next_number)
            next_number = temp_sequence[0] - next_number
...
result = sum_next_number_in_sequence(lines)
print(result)
